...
export async function generateQuotationPDF(data: QuotationData): Promise<void> {
  ...
  const finalY = (autoTableDoc.lastAutoTable?.finalY ?? 88) + 10
...
export async function generatePaymentStatementPDF(data: PaymentStatementData): Promise<void> {
  ...
  const afterBilling = (autoTableDoc.lastAutoTable?.finalY ?? 80) + 10
  ...
  const afterPayments = (autoTableDoc.lastAutoTable?.finalY ?? afterBilling + 5) + 10
