...
export async function GET(req: NextRequest) {
  ...
  const totalCreditsSold = txAgg.reduce((s, a) => s + a.total, 0);
  ...
  const totalAchieved = achAgg.reduce((s, a) => s + a.total, 0);
  ...
  const growthData = await Promise.all(
    fyList.map(async (year) => {
      ...
      const collected = p.reduce((s, x) => s + (Number(x.amountPaid)  || 0), 0);
