...
interface AnnualReturn {
  ...
  client?: {
    ...
    category: string;
    state: string;
    contacts?: { name: string; selectedEmails: string[]; suggestedEmails: string[] }[];
  };
}
...
function buildDraftEmailOptions(contacts: NonNullable<AnnualReturn["client"]>["contacts"] = []) {
  ...
  contacts.forEach((contact) => {
    const selectedEmails = Array.from(new Set((contact.selectedEmails || []).filter(Boolean)));
    const suggestedEmails = Array.from(new Set((contact.suggestedEmails || []).filter(Boolean)));
...
export default function AnnualReturnPage() {
  ...
  const [draftTo, setDraftTo] = useState<EmailOption[]>([]);
  const [draftSuggestions, setDraftSuggestions] = useState<EmailOption[]>([]);
  const [draftSuggestionCatalog, setDraftSuggestionCatalog] = useState<EmailOption[]>([]);
  ...
  const previousDraftToRef = React.useRef<EmailOption[]>([]);
  ...
  React.useEffect(() => {
    const removedRecipients = previousDraftToRef.current.filter(
      (entry) => !draftTo.some((current) => current.email === entry.email)
    );

    if (removedRecipients.length > 0) {
      setDraftSuggestions((current) =>
        removedRecipients.reduce(
          (next, removedEntry) => restoreSuggestion(removedEntry.email, next, draftSuggestionCatalog),
          current
        )
      );
    }

    previousDraftToRef.current = draftTo;
  }, [draftSuggestionCatalog, draftTo]);

  const openAdd  = () => { setEditRecord(null); setForm(createEmptyForm(fy)); setModalOpen(true); };
  ...
  const openDraftModal = async (rec: AnnualReturn) => {
    ...
    const { selected, suggestions } = buildDraftEmailOptions(rec.client?.contacts || []);
    setDraftTo(selected);
    setDraftSuggestions(suggestions);
    setDraftSuggestionCatalog(suggestions);
    setDraftCustomEmail("");
    ...
    if (selected.length > 0) {
      toast.success(
        `Auto-filled ${selected.length} recipient${selected.length !== 1 ? "s" : ""} from linked contact selections`,
        { duration: 3000 }
      );
    } else {
      toast("No selected contact emails found for this client — add recipients manually", { icon: "⚠️" });
    }
  ...
  const addDraftSuggestion = (suggestion: EmailOption) => {
    if (draftTo.find((entry) => entry.email === suggestion.email)) return;
    setDraftTo((prev) => [...prev, suggestion]);
    setDraftSuggestions((prev) => prev.filter((entry) => entry.email !== suggestion.email));
  };
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              {draftTo.length > 0 && (
                <span className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">
                  ✓ {draftTo.length} auto-filled from linked contacts
                </span>
              )}
            </div>
            <div className={`min-h-[44px] w-full rounded-lg border px-3 py-2 flex flex-wrap gap-2 items-center ${draftTo.length === 0 ? "border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/10" : "border-base bg-card"}`}>
              ...
              {draftTo.length === 0 && <span className="text-xs text-amber-500">No selected contact emails found — add manually below</span>}
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {draftSuggestions.map((suggestion) => (
                  <button
                    ...
                    type="button"
                    onClick={() => addDraftSuggestion(suggestion)}
